...
class GoParser {
    ...
    parse(ctx) {
        ...
        if (pkgMatch) {
            ...
            const lineNum = types_1.ParserUtils.getLineNumber(content, pkgMatch.index || 0);
        ...
        while ((blockMatch = importBlockRegex.exec(content)) !== null) {
            const lineNum = types_1.ParserUtils.getLineNumber(content, blockMatch.index);
        ...
        while ((singleMatch = singleImportRegex.exec(content)) !== null) {
            const lineNum = types_1.ParserUtils.getLineNumber(content, singleMatch.index);
            const importPath = singleMatch[2];
            edges.push({
                id: `edge:import:${filePath}:${lineNum}:${importPath}`,
                sourceId: fileNodeId,
        ...
        while ((match = funcRegex.exec(content)) !== null) {
            ...
            types_1.ParserUtils.extractCallsFromBody(funcBody, funcId, filePath, lineNum, edges);
        }
    }
}
