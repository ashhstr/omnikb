...
export class DartParser implements ILanguageParser {
  ...
  public parse(ctx: ParserContext): void {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const classMatch = line.match(
        /^(?:\s*)(?:abstract\s+)?(?:class|mixin|enum|extension)\s+([A-Za-z0-9_$]+)(?:<[^>]+>)?(?:\s+extends\s+([A-Za-z0-9_$<>,\s]+))?(?:\s+with\s+([A-Za-z0-9_$<>,\s]+))?(?:\s+implements\s+([A-Za-z0-9_$<>,\s]+))?/
      );
      ...
      const methodMatch = line.match(
        /^(?:\s*)(?:@override\s+)?(?:(?:Future|Stream|Widget|void|int|double|String|bool|[A-Za-z0-9_$<>]+)\s+)?([A-Za-z0-9_$]+)\s*\(([^)]*)\)(?:\s+async\*?|\s+sync\*?)?\s*(?:=>|\{)/
      );
      ...
      const providerMatch = line.match(
        /^(?:\s*)final\s+([A-Za-z0-9_$]+Provider)\s*=\s*([A-Za-z0-9_$]+)/
      );
      if (providerMatch) {
