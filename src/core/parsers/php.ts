  ...
  public parse(ctx: ParserContext): void {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const classMatch = line.match(
        /^(?:\s*)(?:abstract\s+|final\s+)?(?:class|interface|trait|enum)\s+([A-Za-z0-9_]+)(?:\s+extends\s+([A-Za-z0-9_\\]+))?(?:\s+implements\s+([A-Za-z0-9_\\,\s]+))?/
      );
      ...
      const methodMatch = line.match(
        /^(?:\s*)(?:(?:public|protected|private|static|abstract|final)\s+)*function\s+([A-Za-z0-9_]+)\s*\(([^)]*)\)/
      );
      ...
      if (
        ...
        const relationMatch = body.match(
          /\$this->(hasOne|hasMany|belongsTo|belongsToMany|morphTo|morphMany)\(\s*([A-Za-z0-9_]+)::class/
        );
        if (relationMatch) {
