...
export class JvmParser implements ILanguageParser {
  ...
  public parse(ctx: ParserContext): void {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const classMatch = line.match(
        /^(?:\s*)(?:@\w+(?:\([^)]*\))?\s+)*(?:public|private|protected|internal|abstract|open|final|data|sealed)?\s*(?:class|interface|enum|record|object)\s+([A-Za-z0-9_$]+)(?:<[^>]+>)?(?:\s*:\s*([A-Za-z0-9_$<>,\s()]+)|\s+extends\s+([A-Za-z0-9_$<>,\s]+))?(?:\s+implements\s+([A-Za-z0-9_$<>,\s]+))?/
      );
      ...
      const springRouteMatch = line.match(
      ...
      const methodMatch = line.match(
        /^(?:\s*)(?:@\w+(?:\([^)]*\))?\s+)*(?:public|private|protected|internal|override|final|abstract|open|suspend)?\s*(?:fun\s+|void\s+|[A-Za-z0-9_$<>[\],\s]+\s+)([A-Za-z0-9_$]+)\s*\(([^)]*)\)(?:\s*:\s*[A-Za-z0-9_$<>]+)?\s*(?:\{|=)/
      );
